...
- 태스크 식별자 집합을 푸는 조건이 표면에 있고 상한이 목록 한 장의 상한과 같은가
- 에이전트 프로젝터가 읽는 토픽이 한 벌이고 투영이 원장의 칸을 빠짐없이 채우는가
- 배경 작업의 원장 표가 축의 칸을 갖고 소비자 그룹과 자문 잠금 열쇠가 축마다 갈리는가
- 색인이 문서 식별자와 질의로 두 축을 가르고 색인 본문에 산문이 섞이지 않았는가
- 축을 걸러 읽어야 하는 자리를 케이스가 빠짐없이 열거하는가
